...
Im nächsten Abschnitt wird die Methode "Compare Vertex" verwendet, um sicherzustellen, dass die Nachbarn in einem Knoten des Graphen unterschiedliche Werte haben. Diese Methode vergleicht die Werte der Nachbarn auf Ungleichheit, indem sie die "Compare Vertex"-Methode verwendet, die wir in einem kommenden Abschnitt genauer erläutern werden. Dadurch wird sichergestellt, dass die Regeln des Sudokus eingehalten werden und eine gültige Lösung gefunden werden kann. Die Vergleiche werden durch eine "for"-Schleife durchgeführt.
